...
/*   Updated: 2023/08/02 10:46:59 by meharit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_angle(char c, int y, int x, t_mlx *m)
{
	if (m->p.angle != -1)
	...
	m->p.x = x * m->map.tile + (m->map.tile / 2);
	m->p.y = y * m->map.tile + (m->map.tile / 2);
}

void	pars_character(char *line, int y, t_mlx *m)
{
	int	i;
...
void	valid_line(char *line, t_mlx *m, int index)
{
	...
	while (line[i])
	{
		if ((line[i] == '0' || line[i] == 'D' || start_pos(line[i]))
